...
| | `php artisan tibia:clean-wiki-residue` (once — strips leftover TibiaWiki template markup from text imported by older passes: items whose description is literally `}}`, travelling NPCs ending in `{{TransportList \| \| \|`, spawn locations cut off at `{{Mapper Coords\|…`. Run `--dry-run` first; safe to re-run, and new imports are scrubbed automatically) |
